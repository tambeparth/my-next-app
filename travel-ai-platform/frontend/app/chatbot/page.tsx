            ...
            if (messages.length === 0 && !currentChatId) {
                try {
                    ...
                    try {
                        ...
                        // Use default welcome message if API fails
                    }
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error.message?.includes('401')) {
                errorText = "❌ Authentication error. Please log in and try again.";
            } else if (error.message?.includes('500')) {
                errorText = "❌ Server error. The AI service is temporarily unavailable.";
            } else if (error.message?.includes('Failed to fetch')) {
                errorText = "❌ Connection error. Please check if the backend servers are running.\n\nTo start servers:\n1. Backend: cd backend-v2 && npm run dev\n2. Chatbot: cd backend-chatbot && python main.py";
            }
...
export default function ChatbotPageWithAuth() {
    return (
        <AuthGuard fallback={<LoginRequired />}>
            <SmartAITravelChatbot />
        </AuthGuard>
    );
}
